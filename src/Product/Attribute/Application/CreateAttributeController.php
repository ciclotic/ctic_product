...
use CTIC\Product\Attribute\Infrastructure\Controller\AttributeController;
use CTIC\Product\Attribute\Infrastructure\Form\Type\AttributeType;
...
class CreateAttributeController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new AttributeType(Attribute::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Product\Attribute\Infrastructure\Form\Type\AttributeType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateAttribute();
        }
        if(empty($command->resource))
        {
            $command->resource = Attribute::class;
        }
        if(empty($command->command))
        {
            $command->command = new AttributeCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return AttributeController::class;
    }
