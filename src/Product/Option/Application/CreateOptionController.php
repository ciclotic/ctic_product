...
use CTIC\Product\Option\Infrastructure\Controller\OptionController;
use CTIC\Product\Option\Infrastructure\Form\Type\OptionType;
...
class CreateOptionController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new OptionType(Option::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Product\Option\Infrastructure\Form\Type\OptionType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateOption();
        }
        if(empty($command->resource))
        {
            $command->resource = Option::class;
        }
        if(empty($command->command))
        {
            $command->command = new OptionCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return OptionController::class;
    }
